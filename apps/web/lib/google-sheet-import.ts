...
export async function importWorkItemsFromGoogleSheet(
  ...
  const sourceRows = await readGoogleSheetRows(request, deps.fetchImpl ?? fetch, deps.env);
  ...
  if (mode === "replace") {
    for (const node of planning.nodes) {
      result.actions.push({
        ...
        title: node.title,
        parentPath: node.parentTempId ? pathLabel(node.pathSegments.slice(0, -1)) : null,
  ...
  const existingTree = await deps.repository.listTree(workspaceId as WorkspaceId);
  ...
  for (const node of planning.nodes) {
    ...
    const parentId = node.parentTempId ? resolvedNodeIds.get(node.parentTempId) ?? null : null;
    ...
    if (candidates.length === 1) {
      ...
      const needsMove =
        existing.parentId !== parentId || existing.siblingOrder !== node.siblingOrder;
      if (!needsMove) {
        result.actions.push({
          action: "skip",
          path: pathLabel(node.pathSegments),
...
function indexExistingNodes(tree: WorkTreeReadNode[]): Map<string, ExistingNode[]> {
...
function buildPlan(rows: ParsedRow[]): {
  ...
  const explicitNodes = new Map<string, { rowNumber: number; order: number | null }>();
  const titleByKey = new Map<string, string>();

  const rowsWithPath = validRows.filter((row) => row.path && row.path.trim().length > 0);
  ...
  for (const row of rowsWithPath) {
    ...
    const fullSegments = pathLast === normalizedTitle ? parsedSegments : [...parsedSegments, row.title];
  ...
  for (const resolved of parentRowsValidation.rows) {
    ...
    explicitNodes.set(key, { rowNumber: resolved.rowNumber, order: resolved.order });
...
function validateParentTitleRows(rows: ParsedRow[]): {
  rows: Array<{ title: string; rowNumber: number; order: number | null; pathSegments: string[] }>;
  errors: ImportError[];
} {
  const errors: ImportError[] = [];
  if (rows.length === 0) {
...
function resolvePathFromParentChain(row: ParsedRow, byTitle: Map<string, ParsedRow>): string[] {
...
function parseRows(rows: string[][]): ParsedRow[] {
  ...
  const orderIndex = findHeaderIndex(headerRow, ["order", "siblingorder", "sibling_order"]);
  const levelIndex = findHeaderIndex(headerRow, ["level", "уровень"]);

  if (parentTitleIndex < 0 && pathIndex < 0 && levelIndex < 0) {
    throw new Error("Sheet must include either \"parentTitle\", \"path\" or \"level\" column");
  }
  ...
  for (let i = 1; i < rows.length; i += 1) {
    ...
    const parentTitle =
      parentTitleIndex >= 0 ? (row[parentTitleIndex] ?? "").trim() || null : null;
    ...
    parsed.push({ rowNumber, title, parentTitle, path, order, level, parseError: null });
  }
...
async function readGoogleSheetRows(
  ...
  if (!valuesResponse.ok) {
    throw new Error(`Failed to read sheet values: HTTP ${valuesResponse.status}`);
  }
...
function buildCsvExportUrl(sheetUrl: string | undefined, sheetId: string): string {
  if (!sheetUrl) {
...
function readSheetsCredentials(env: NodeJS.ProcessEnv): SheetsCredentials | null {
...
async function readFirstSheetTitle(
  ...
  if (!response.ok) {
    throw new Error(`Failed to read spreadsheet metadata: HTTP ${response.status}`);
  }
