...
- режим `split`/`table-only` хранится в пределах browser tab session и после refresh восстанавливается для текущего workspace.
...
- editing context минимум включает:
  ...
  - его direct children, если они есть;
  - если direct children отсутствуют, контекст расширяется вверх по цепочке parent-узлов;
...
- при автоматическом фокусировании масштаб выбирается адаптивно в диапазоне `60%..85%` в зависимости от размеров editing context;
- редактируемый узел должен попадать примерно в вертикальный центр видимой области mindmap;
- по горизонтали автопозиционирование должно оставлять пространство для видимости parent-связи и максимально возможного числа дочерних веток без сложной оптимизационной логики;
